---
title: "Step 01 PUMS to Database"
author: "David Ory"
output: 
  html_document:
    theme: cosmo
...
#### Purpose
Prepares PUMS database inputs for the Population Synthesizer. These procedures replace previous procedures done in SQL. Note the input geographies file use the 2000 Decennial Census PUMA boundaries, as do the five-year 2007 to 2011 PUMS data.  

#### Outputs
1.  Year-specific household and person PUMS data written to MySQL database

#### TODO
1.  Everything, early days
2.  Put in the 2000 data sufficient for Step 02 (building the controls)
3.  Put in the 2000 data sufficient for running the software
...
PUMS_0711_DIR <- "M:/Development/PopulationSynthesizer/PopSyn3/pums_data/acs2011_5yr/ca"

PUMS_0711_HH_FILE <- paste(PUMS_0711_DIR, "ss11hca.csv", sep = "/")
PUMS_0711_PER_FILE <- paste(PUMS_0711_DIR, "ss11pca.csv", sep = "/")

PUMS_00_DIR <- "M:/Development/PopulationSynthesizer/PopSyn3/pums_data/_working"
PUMS_00_FILE <- paste(PUMS_00_DIR, "usa_00001.csv", sep = "/")
...
input_pums_00 <- read.csv(PUMS_00_FILE, header = TRUE)
```
...
pums_hh  <- input_pums_0711_hh[input_pums_hh$PUMA %in% relevant_pumas,]
pums_per <- input_pums_0711_per[input_pums_per$PUMA %in% relevant_pumas,]
...
pums_hh  <- input_pums_00[input_pums_00$PUMA %in% relevant_pumas,]

#START HERE
# build the hh file
# build the person file
# check the worker and occupation variables
# just put those in the database for now (sufficient for Step 02)
# TODO get all the fields from SI and re-do
