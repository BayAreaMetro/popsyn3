...
#### Outputs
1.  Control tables (maz, taz, and county) written to MySQL database
2.  Household and Person PUMS data written to MySQL database

#### TODO
1. Everything, early days
2. See all the inline TODOs
3. Re-build the controls from census data, use scripts and apis
4. Give the controls year specific names in MySQL and then the configuration scripts
...
# bring in income
# TODO verify what dollar year the TAZ controls are in
# TODO ideally move the creation of the inputs from Census API into a script
...
# TODO make sure this year dollars and TAZ controls align
pums_hh <- pums_hh %>%
...
# TODO make the SOC to occupation relationship a table
pums_per <- pums_per %>%
...
dbWriteTable(conn = mysql_connection, name = paste('household_table', YEAR, sep = '_'), value = as.data.frame(pums_hh),  overwrite = TRUE)
dbWriteTable(conn = mysql_connection, name = paste('person_table', YEAR, sep = '_'),    value = as.data.frame(pums_per), overwrite = TRUE)
