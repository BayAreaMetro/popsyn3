############################################################################################################
# Script to build 2005 controls using 2000 controls
#
# All controls are scaled in proportion to rate at which households change between 2000 and 2005
# Number of HHs are available at MAZ levelf or both 2000 and 2005
#
# Author: binny.mathewpaul@rsginc.com Feb 2017
###########################################################################################################
message("=== Running buildControls2005.R")
...
# get HHs at all geographic levels for 2000
maz_hh_00 <- maz2000[, c("maz_original","HH")]
colnames(maz_hh_00) <- c("MAZ_ORIGINAL", "HH00")
...
county_hh_00 <- maz_hh_00 %>%
  left_join(geogXWalk[,c("MAZ_ORIGINAL", "MTCCountyID")], by = "MAZ_ORIGINAL") %>%
  group_by(MTCCountyID) %>%
  summarise(HH00 = sum(HH00)) %>%
  ungroup()

# get HHs at all geographic levels for 2005
maz_hh_05 <- maz2005Data[, c("MAZ", "X2005.HH")]
colnames(maz_hh_05) <- c("MAZ_ORIGINAL", "HH05")
...
maz_factor <- maz_hh_00 %>%
  left_join(maz_hh_05, by = "MAZ_ORIGINAL") %>%
  mutate(factor = ifelse(HH00>0, HH05/HH00, 0)) %>%
  mutate(case_zero = ifelse(HH00==0 & HH05>0, 1, 0))

taz_factor <- taz_hh_00 %>%
  left_join(taz_hh_05, by = "TAZ_ORIGINAL") %>%
  mutate(factor = ifelse(HH00>0, HH05/HH00, 0)) %>%
  mutate(case_zero = ifelse(HH00==0 & HH05>0, 1, 0))

county_factor <- county_hh_00 %>%
  left_join(county_hh_05, by = "MTCCountyID") %>%
  mutate(factor = ifelse(HH00>0, HH05/HH00, 0))

# compute county level control totals of MAZ and TAZ controls
mazCountyControls <- maz2000 %>%
